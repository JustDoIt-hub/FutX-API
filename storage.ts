...
export class DatabaseStorage {
  ...
  async getUserByTelegramId(userId: number) {
    return (await db.select().from(users).where(eq(users.userId, userId)))[0];
  }

  async createUser(data: { userId: number; telegram_username?: string; coins: number; }) {
    console.log("[DEBUG] About to insert user with data:", data);
    return (await db.insert(users).values({
      userId: data.userId,
      playerId: 1, // default starter player
      quantity: 1,
  ...
  async updateUser(userId: number, data: Partial<User>) {
    return (await db.update(users).set(data).where(eq(users.userId, userId)).returning())[0];
  }
