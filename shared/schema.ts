...
import { pgTable, serial, integer, varchar, timestamp, boolean, text, pgEnum, bigint } from "drizzle-orm/pg-core";
import { createInsertSchema } from "drizzle-zod";
import { relations } from "drizzle-orm";
import { z } from "zod";
import 'dotenv/config';
...
export const players = pgTable("players", {
  ...
  isManager: boolean("is_manager").default(false),
  isGk: boolean("is_gk").default(false),
  position: positionEnum("position").notNull(),  // Use pgEnum for position column
});
...
export const insertUserPlayerSchema = createInsertSchema(users).pick({
  userId: true,
  playerId: true,
  quantity: true,
}).extend({
